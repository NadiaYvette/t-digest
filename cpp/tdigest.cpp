...
void TDigest::mergeIntoLast(std::vector<Centroid>& centroids, const Centroid& c) {
    ...
    for (size_t i = 0; i < centroids_.size(); i++) {
        ...
        if (i == centroids_.size() - 1) {
            if (target > n - c.weight / 2.0) {
                if (c.weight == 1.0) return max_;
                double remaining = n - c.weight / 2.0;
                return c.mean + (max_ - c.mean) * ((target - remaining) / (c.weight / 2.0));
            }
            return c.mean;
        }
