...
main = do
  ...
  mapM_ (\i -> do
    ...
    check ref (printf "N=%d  ratio=%.2f (expected ~%.1f)" (sizes !! i) ratio expected)
  ...
  mapM_ (\n -> do
    ...
    check ref (printf "N=%-9d  centroids=%-4d  (delta=%.0f, limit=%d)" n cc delta (5 * round delta :: Int))
              (cc <= 5 * round delta)
    ) sizes
  ...
  mapM_ (\i -> do
    ...
    check ref (printf "N=%d  ratio=%.2f (expected ~1.0)" (querySizes !! i) ratio)
  ...
  mapM_ (\q -> do
    ...
    mapM_ (\i -> do
      ...
      check ref (printf "delta=%.0f q=%.3f error decreases (%.6f <= %.6f)"
                        (deltas !! i) q (errors !! i) (errors !! (i-1)))
                ok
      ) [1..length deltas - 1]
    ) tailQs
  ...
  let nMerge = 10000 :: Int
      td1_0 = foldl' (\td i -> TDigest.add (fromIntegral i / fromIntegral nMerge) td)
                      (TDigest.emptyWith 100) [0..nMerge `div` 2 - 1]
      td2_0 = foldl' (\td i -> TDigest.add (fromIntegral i / fromIntegral nMerge) td)
                      (TDigest.emptyWith 100) [nMerge `div` 2 .. nMerge - 1]
  ...
  compressTimes <- mapM (\bufN -> do
    let buf = map (\i -> let v = fromIntegral i / fromIntegral bufN
                         in v) [0..bufN-1]
  ...
  mapM_ (\i -> do
    ...
    check ref (printf "buf_n=%d  ratio=%.2f (expected ~%.1f)" (compressSizes !! i) ratio expected)
              ok
