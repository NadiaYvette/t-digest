...
void tdigest_compress(tdigest_t *td) {
    ...
    memcpy(all + td->centroid_count, td->buffer, td->buffer_count * sizeof(centroid_t));
    td->buffer_count = 0;
    ...
    if (td->centroid_cap < all_count) {
        ...
        td->centroids = (centroid_t *)realloc(td->centroids, td->centroid_cap * sizeof(centroid_t));
    }
...
double tdigest_quantile(tdigest_t *td, double q) {
    if (td->buffer_count > 0) tdigest_compress(td);
    if (td->centroid_count == 0) return NAN;
    if (td->centroid_count == 1) return td->centroids[0].mean;
    ...
    for (int i = 0; i < td->centroid_count; i++) {
        ...
        if (i == 0) {
            if (x < c->mean) {
                ...
                double frac = (c->mean == td->min) ? 1.0 : (x - td->min) / (c->mean - td->min);
        ...
        if (i == td->centroid_count - 1) {
            if (x > c->mean) {
                ...
                double frac = (td->max == c->mean) ? 0.0 : (x - c->mean) / (td->max - c->mean);
