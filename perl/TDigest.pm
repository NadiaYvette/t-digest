    ...
    for my $i (0 .. $count - 1) {
        ...
        if ($i == 0) {
            if ($target < $cweight / 2.0) {
                ...
                return $self->{min} + ($cmean - $self->{min}) * ($target / ($cweight / 2.0));
            }
        }
        ...
        if ($i == $count - 1) {
            if ($target > $n - $cweight / 2.0) {
                ...
                return $cmean + ($self->{max} - $cmean) * (($target - $remaining) / ($cweight / 2.0));
            }
            return $cmean;
        }
    ...
    for my $i (0 .. $count - 1) {
        ...
        if ($i == 0) {
            if ($x < $cmean) {
                ...
                my $frac = ($cmean == $self->{min}) ? 1.0 : ($x - $self->{min}) / ($cmean - $self->{min});
        ...
        if ($i == $count - 1) {
            if ($x > $cmean) {
                ...
                my $frac = ($self->{max} == $cmean) ? 0.0 : ($x - $cmean) / ($self->{max} - $cmean);
